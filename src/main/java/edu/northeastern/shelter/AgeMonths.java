 ...
 *
 * <p>
 * This is a <em>value object</em>: it wraps a single number, but it wraps it in
 * a way that makes
 * an invalid age impossible to construct. Once you have an {@code AgeMonths} in
 * your hand, you know
 * it is non-negative and within a plausible range — you never have to check
 * again.
 *
 * <p>
 * The class is <strong>immutable</strong>: the field is {@code private final},
 * there is no
 * setter, and no method changes the object. {@link #of(int)} and
 * {@link #months()} ship complete as
...
public final class AgeMonths {
   ...
   *
   * <p>
   * Making the constructor private and offering a named factory method is a habit
   * worth
   * acquiring: the factory can validate, can be given a meaningful name, and
   * could one day return a
   ...
   *
   * <p>
   * Ships complete as the worked example. Read it before you write anything else:
   * validate
   ...
   * @throws IntakeException if {@code months} is negative or exceeds
   *                         {@link #MAX_MONTHS}
   */
   ...
   *
   * <p>
   * An age of 23 months is one complete year, not two: partial years are
   * discarded.
   *
   ...
   *
   * <p>
   * An age of 23 months is one year and eleven months, so this returns 11.
   *
   ...
   *
   * <p>
   * The exact format is specified by the provided tests, and it is fussy on
   * purpose — reading a
   ...
   *
   * <p>
   * Note the singular/plural agreement, and that a whole number of years omits
   * the months part
   * entirely.
   *
